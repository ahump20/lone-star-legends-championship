...
import {
  ...
  createGlassMaterial,
  createGrassTexture,
  createInfieldTexture,
  createSeatingMaterial,
  createSteelMaterial,
} from './materials';
import type { BuschStadiumSceneOptions, RenderHooks, TimeOfDay } from './types';
...
function createSeatingBowl(): Group {
  const group = new Group();
...
function createDowntownBackdrop(): Group {
  const group = new Group();
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    for (let y = 2; y < windowCountY; y += 2) {
      for (let x = 0; x < windowCountX; x += 1) {
        ...
        window.position.set(
          building.position.x - width / 2 + 1.5 + x * 3,
          building.position.y - height / 2 + 3 + y * 3,
...
function createGatewayArch(): Mesh {
...
interface ScoreboardInstance {
  group: Group;
  update: (data: BuschStadiumSceneOptions['scoreboard']) => void;
}

function createScoreboard(initialState: BuschStadiumSceneOptions['scoreboard']): ScoreboardInstance {
  const group = new Group();
  ...
  const update = (state: BuschStadiumSceneOptions['scoreboard']): void => {
...
function createLightMasts(): Group {
  const group = new Group();
...
interface CrowdInstance {
  group: Group;
  update: (density: number, palette: string[]) => void;
}

function createCrowd(density: number, palette: string[]): CrowdInstance {
  const group = new Group();
  ...
  const update = (densityValue: number, paletteValue: string[]): void => {
...
function applyAtmosphere(scene: Scene, timeOfDay: TimeOfDay): void {
  if (timeOfDay === 'day') {
...
export function buildBuschStadiumScene(
  canvas: HTMLCanvasElement,
  options: BuschStadiumSceneOptions,
): { scene: Scene; camera: PerspectiveCamera; controls: OrbitControls; hooks: RenderHooks } {
  const scene = new Scene();
  let currentOptions: BuschStadiumSceneOptions = { ...options };
  ...
  const palette = currentOptions.crowd.palette.map((value: ColorRepresentation) => new Color(value).getStyle());
  ...
  const sun = new DirectionalLight(currentOptions.lighting.color as string, currentOptions.lighting.intensity);
  const positionSun = (spread: number): void => {
  ...
  currentOptions.crowd.palette.forEach((color: ColorRepresentation, index: number) => {
    const point = new PointLight(color as string, 0.3, 80, 1.6);
  ...
  const update = (deltaMs: number): void => {
    ...
    lightArray.children.forEach((light: Object3D, index: number) => {
  ...
  const setOptions = (next: BuschStadiumSceneOptions): void => {
    currentOptions = { ...next };
    applyAtmosphere(scene, currentOptions.timeOfDay);
    skyUniforms.turbidity.value = currentOptions.timeOfDay === 'day' ? 2 : 6;
    skyUniforms.rayleigh.value = currentOptions.timeOfDay === 'night' ? 0.5 : 2.5;
    ...
    const updatedPalette = currentOptions.crowd.palette.map((value: ColorRepresentation) => new Color(value).getStyle());
    ...
    positionSun(currentOptions.lighting.spread);
    lightArray.children.forEach((child: Object3D, index: number) => {
