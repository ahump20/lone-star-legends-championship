...
export interface PlayerAttributes {
  ...
  // Pitching attributes (0-100)
  velocity?: number;
  control?: number;
  stamina?: number;
  movement?: number;
  
  // Fielding attributes (0-100)
  fielding: number;
  armStrength: number;
  range: number;
...
export class MLBDataLoader {
  private teams: Map<string, MLBTeam> = new Map();
  private players: Map<string, MLBPlayer> = new Map();
  private dataPath: string = '/data/ingested/mlb/';
  
  constructor() {
    console.log('⚾ MLB Data Loader initialized');
  }
  
  public async loadTeamData(teamId: string): Promise<MLBTeam | null> {
    try {
      // Check cache first
  ...
  private convertToGameTeam(teamId: string, rosterData: any, statsData: any): MLBTeam {
    const teamInfo = this.getTeamInfo(teamId);
    const roster: MLBPlayer[] = [];
  ...
  private convertToMLBPlayer(playerData: any, statsData: any, teamId: string): MLBPlayer {
    // Find player stats if available
    const playerStats = statsData?.players?.find((p: any) => p.id === playerData.id);
  ...
  private calculatePlayerAttributes(player: any, stats: any): PlayerAttributes {
  ...
  private calculateBatterAttributes(player: any, stats: any): PlayerAttributes {
  ...
  private calculatePitcherAttributes(player: any, stats: any): PlayerAttributes {
  ...
  private getFieldingByPosition(position: string): number {
  ...
  private getArmStrengthByPosition(position: string): number {
    const armMap: Record<string, number> = {
  ...
  private getRangeByPosition(position: string): number {
  ...
  private getTeamInfo(teamId: string): any {
  ...
  private generateDefaultTeam(teamId: string): MLBTeam {
    const teamInfo = this.getTeamInfo(teamId);
    const roster: MLBPlayer[] = [];
  ...
  private generateDefaultPlayer(position: string, number: number, teamId: string, isPitcher: boolean): MLBPlayer {
    const names = [
  ...
  public async loadAllTeams(): Promise<MLBTeam[]> {
    const teamIds = [
  ...
  public getPlayer(playerId: string): MLBPlayer | undefined {
    return this.players.get(playerId);
  }
  
  public getTeam(teamId: string): MLBTeam | undefined {
    return this.teams.get(teamId);
  }
  
  public getAllPlayers(): MLBPlayer[] {
    return Array.from(this.players.values());
  }
  
  public getAllTeams(): MLBTeam[] {
