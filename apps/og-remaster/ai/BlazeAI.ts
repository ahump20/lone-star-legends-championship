...
interface AIPersonality {
  name: string;
  aggression: number; // 0-1 (conservative to aggressive)
  patience: number; // 0-1 (swings often to waits for perfect pitch)
  intelligence: number; // 0-1 (random to strategic)
  clutchFactor: number; // 0-1 (chokes to thrives under pressure)
  specialties: string[]; // ['power', 'contact', 'speed', 'defense']
}

interface PitchDecision {
  type: 'fastball' | 'curveball' | 'slider' | 'changeup' | 'knuckleball';
  location: { x: number; y: number }; // Strike zone coordinates
  velocity: number; // 0-1 (slow to fast)
  confidence: number; // 0-1 (AI's confidence in this pitch)
}

interface SwingDecision {
  shouldSwing: boolean;
  timing: 'early' | 'perfect' | 'late';
  power: number; // 0-1 (contact to power swing)
...
export class BlazeAI {
  ...
  private pitchHistory: PitchDecision[] = [];
  private swingHistory: SwingDecision[] = [];
  ...
  constructor(personality: AIPersonality, gameState: GameState) {
  ...
  // PITCHING AI
  public decidePitch(): PitchDecision {
  ...
  private selectPitchType(
    context: GameContext, 
    batterTendencies: Map<string, number>
  ): PitchDecision['type'] {
    ...
    if (Math.random() > intelligence) {
      ...
      const types: PitchDecision['type'][] = ['fastball', 'curveball', 'slider', 'changeup'];
    ...
    if (strikes > balls) {
      ...
      const breakingBalls: PitchDecision['type'][] = ['curveball', 'slider', 'changeup'];
  ...
  private calculatePitchVelocity(
    context: GameContext,
    pitchType: PitchDecision['type']
  ): number {
  ...
  // BATTING AI
  public decideSwing(pitch: PitchDecision): SwingDecision {
    const context = this.getGameContext();
    ...
    let timing: SwingDecision['timing'] = 'perfect';
    let power = 0.5;
  ...
  private evaluatePitch(pitch: PitchDecision): number {
  ...
  private shouldSwingAtPitch(
    ...
    this.updatePatternMemory(
      `swing_decision_${pitch.type}_${context.count.balls}-${context.count.strikes}`,
      shouldSwing ? pitchQuality : 1 - pitchQuality
    );
    
    return shouldSwing;
  }

  private calculateSwingTiming(pitch: PitchDecision): SwingDecision['timing'] {
    const intelligence = this.personality.intelligence;
  ...
  private calculateSwingPower(pitch: PitchDecision, context: GameContext): number {
    let power = 0.5;
  ...
  private calculateSwingConfidence(
    shouldSwing: boolean,
    pitchQuality: number,
    context: GameContext
  ): number {
  ...
  // BASERUNNING AI
  public decideBaserunning(): {
    steal: boolean;
    advance: 'conservative' | 'normal' | 'aggressive';
    tagUp: boolean;
  } {
    const context = this.getGameContext();
    ...
    let advance: 'conservative' | 'normal' | 'aggressive' = 'normal';
  ...
  // PATTERN RECOGNITION
  private analyzeBatterTendencies(): Map<string, number> {
    const tendencies = new Map<string, number>();
  ...
  private updatePatternMemory(key: string, value: number): void {
  ...
  private getGameContext(): GameContext {
    ...
    let gamePhase: GameContext['gamePhase'] = 'early';
    if (inning <= 3) {
      gamePhase = 'early';
    } else if (inning <= 6) {
      gamePhase = 'middle';
  ...
  // ADAPTIVE DIFFICULTY
  public adjustDifficulty(playerPerformance: {
    wins: number;
    losses: number;
    averageScore: number;
  }): void {
