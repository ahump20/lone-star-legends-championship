...
interface CommentaryLine {
  id: string;
  text: string;
  emotion: 'calm' | 'excited' | 'intense' | 'disappointed' | 'celebratory';
  priority: number; // 1-10, higher plays first
  conditions: CommentaryCondition[];
  audioFile?: string;
  cooldown: number; // Milliseconds before can play again
  lastPlayed?: number;
}

interface CommentaryCondition {
  type: 'event' | 'score' | 'inning' | 'count' | 'runners' | 'player';
...
export class CommentaryEngine {
  ...
  private loadCommentaryDatabase(): void {
  ...
  public async commentate(situation: GameSituation): Promise<void> {
    if (!this.enabled || this.isSpeaking) return;

    // Find applicable commentary lines
    const applicable = this.commentaryLines.filter(line => 
      this.checkConditions(line.conditions, situation) &&
  ...
  private checkConditions(conditions: CommentaryCondition[], situation: GameSituation): boolean {
  ...
  private getSituationValue(type: string, situation: GameSituation): any {
    switch (type) {
      case 'event':
  ...
  private checkCooldown(line: CommentaryLine): boolean {
    if (!line.lastPlayed) return true;
  ...
  private async speak(text: string, emotion: CommentaryLine['emotion']): Promise<void> {
  ...
  public async commentateInstant(text: string, emotion: CommentaryLine['emotion'] = 'excited'): Promise<void> {
  ...
  public generateStatCommentary(stats: {
  ...
  public addCustomLine(line: CommentaryLine): void {
