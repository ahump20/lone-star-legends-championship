...
class BackyardBaseballDemo {
     ...
     */
    setupGameCallbacks() {
        this.gameState.onPitchResult = (result) => {
            console.log('Pitch result:', result.type, result.hitType);
        };
        this.gameState.onScoreUpdate = (home, away) => {
            console.log(`Score Update - Home: ${home}, Away: ${away}`);
        };
        this.gameState.onInningChange = (inning, half) => {
            console.log(`Inning Change - ${half} of ${inning}`);
        };
        this.gameState.onGameOver = (winner) => {
    ...
    renderScoreboard() {
        ...
        if (this.gameState.homeTeam && this.gameState.awayTeam) {
            ...
            ctx.font = 'bold 16px Arial';
            ctx.textAlign = 'center';
            ctx.fillText(`${this.gameState.inningHalf.toUpperCase()} ${this.gameState.inning}`, 475, 75);
        }
    }
    /**
    ...
    renderGameOver() {
        ...
        if (this.gameState.winner) {
            ...
            this.ctx.fillText(`${this.gameState.awayTeam.score} - ${this.gameState.homeTeam.score}`, 512, 420);
        }
        // Play again
        this.ctx.fillStyle = '#FFD700';
