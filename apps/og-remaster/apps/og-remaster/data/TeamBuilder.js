...
export class TeamBuilder {
     ...
     */
    createBalancedTeam(teamId, playerIds) {
        const teamData = this.teams.find(t => t.id === teamId);
        if (!teamData)
            return null;
        const roster = [];
        const selectedPlayers = playerIds.map(id => this.characters.find(c => c.id === id)).filter(p => p);
