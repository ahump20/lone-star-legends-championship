import { CanvasTexture, Color, MeshPhysicalMaterial, MeshStandardMaterial, RepeatWrapping, Vector2, } from 'three';
...
export function createGrassTexture() {
    return createCanvasTexture(1024, 1024, (ctx) => {
        ...
        ctx.globalAlpha = 1;
        const radialGradient = ctx.createRadialGradient(ctx.canvas.width / 2, ctx.canvas.height / 2, 0, ctx.canvas.width / 2, ctx.canvas.height / 2, ctx.canvas.width / 2);
...
export function createInfieldTexture() {
    return createCanvasTexture(1024, 1024, (ctx) => {
...
export function createSeatingMaterial(primaryColor, secondaryColor) {
...
export function createGlassMaterial(color, opacity = 0.55) {
    return new MeshPhysicalMaterial({
        color,
        roughness: 0.05,
        metalness: 0,
