...
export class TournamentMode {
    ...
    startMatch(matchId) {
        ...
        this.gameState.maxInnings = this.settings.innings;
        console.log(`⚾ Match started: ${match.team1?.name} vs ${match.team2?.name}`);
        return true;
    }
    completeMatch(matchId, score) {
        const match = this.findMatch(matchId);
    ...
    advanceWinner(match) {
        ...
        if (nextRoundIndex >= this.bracket.matches.length)
            return;
    ...
    addHighlight(inning, description, importance) {
