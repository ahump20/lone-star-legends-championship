...
import { MLBDataLoader } from './data/MLBDataLoader';
...
class OGRemasterGame {
    ...
    setupEventListeners() {
        ...
        this.inputManager.on('swing', (result) => {
            this.handleSwing(result);
        });
        this.inputManager.on('pitch', (result) => {
    ...
    setupGameplayCallbacks() {
        ...
        this.gameState.swingBat = () => {
            ...
            if (result.contact) {
                let hitStrength = 'solid';
                if (result.result === 'homerun')
                    hitStrength = 'crushing';
    ...
    applyTeamToGameState(team) {
        ...
        if (this.gameState) {
            ...
            const lineup = team.roster
                ...
                .slice(0, 9);
            this.gameState.homeLineup = lineup;
            // Set starting pitcher
            const pitcher = team.roster.find(p => p.position === 'SP') || team.roster[0];
            this.gameState.currentPitcher = pitcher.name;
            // Set first batter
            if (lineup.length > 0) {
                this.gameState.currentBatter = lineup[0].name;
            }
    ...
    setVolume(master, sfx, commentary, music) {
        this.audioEngine.setMasterVolume(master);
        if (sfx !== undefined)
            this.audioEngine.setSFXVolume(sfx);
        if (commentary !== undefined)
            this.audioEngine.setCommentaryVolume(commentary);
        if (music !== undefined)
            this.audioEngine.setMusicVolume(music);
    }
}
